...
// EncryptWithHeader adds a file to the zip file using the provided name.
// It returns a Writer to which the file contents should be written. File
// contents will be encrypted with AES-256 using the given password. The
// file's contents must be written to the io.Writer before the next call
// to Create, CreateHeader, or Close.
func (w *Writer) EncryptWithHeader(name string, password string, enc EncryptionMethod, fileHeader *FileHeader) (io.Writer, error) {
	fileHeader.SetPassword(password)
	fileHeader.setEncryptionMethod(enc)
